...
import { getRecentPost } from "../../helper/serviceHelper";
                ...
                <div className="sidebar">
                    <div className="sidebar_widgets mb-xl-30">
                        ...
                        <div className="listing-categories">
                            <ul className="custom">
                                ...
                                {category.map((item, i) => (
                                    <li key={i}>
                                        <Link to={"/service/cat/" + item.id} className="text-theme fs-14">{item.title}</Link>
                                    </li>
                                ))}
                                {/* Data */}
                            </ul>
                                    ...
                                    <div className="post">
                                        <div className="post-wrapper">
                                            ...
                                            <div className="popular_post_title">
                                                <h6><Link to={"/service-details/" + item.id} className="text-custom-black">{item.title}</Link></h6>
                    ...
                    <div className="sidebar_widgets">
                        ...
                        <div className="tags">
                            {/* Data */}
                            {tags.map((item, i) => (
                                <Link to={"/service/tag/" + item.id} key={i}>{item.title}</Link>
                            ))}
                            {/* Data */}
                        </div>
                    </div>
